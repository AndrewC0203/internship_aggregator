...
// CLI entrypoint for the board discovery job: `npm run discover -- [--crawl <id>] [--limit <n>]`
...
async function main(): Promise<void> {
  ...
  console.log(
    `Discovery run starting (${crawlId ?? "latest crawl"}${limit ? `, limit ${limit}` : ", uncapped"})...`,
  );

  const summaries = await runDiscovery({ crawlId, limit });
